...
class VideoEncoder extends Emitter
{
	...
	constructor(codec,params)
	{
		...
		//Set codec
		if (!encoder.SetVideoCodec(codec,parseInt(params.width),parseInt(params.heigth),parseInt(params.fps),parseInt(params.bitrate),parseInt(params.intraPeriod),properties))
			//Error
	...
	createIncomingStreamTrack()
	{
		...
		//Add track
		const track = new VideoEncoderIncomingStreamTrack(source); 
